...
- The `Verify version sync` job now installs Python and ruff before running
  `check-spec.js`. That job set up Node only, and `gen-contract.js` at the time
  silently fell back to unformatted output when ruff was missing — so it
  regenerated a differently-formatted `contract_gen.py` and reported drift
