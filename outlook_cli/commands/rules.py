    ...
    if rule.actions:
        ...
        if getattr(a, "forward_as_attachment_to_recipients", None):
            actions["forward_as_attachment_to"] = [
                m.email_address for m in a.forward_as_attachment_to_recipients if m.email_address
            ]
...
def _import_rule_classes():
    ...
    try:
        ...
        from exchangelib.properties import RuleActions, RuleConditions, RulePredicates
...
def _build_exc_kwargs(**kwargs):
    ...
    if kwargs.get("except_sender"):
        result["contains_sender_strings"] = [s.strip() for s in kwargs["except_sender"].split(",")]
    ...
    if kwargs.get("except_keyword"):
        result["contains_body_strings"] = [s.strip() for s in kwargs["except_keyword"].split(",")]
    return result
